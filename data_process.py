...
class DataProcessor(object):
    ...
    def get_ego_agent(self):
        self.anchor_ego_state = self.scenario.initial_ego_state # 该场景最开始的自车初始状态
        ...
        ) #自车过去的轨迹 2s，20个点，相当于100ms一个点

        sampled_past_ego_states = list(past_ego_states) + [self.anchor_ego_state] #拼接初始状态和历史轨迹
        past_ego_states_tensor = sampled_past_ego_states_to_tensor(sampled_past_ego_states) # 自车过去的状态转换成张量 21 * 7
        """
        状态量如下： 
        ...
        past_time_stamps_tensor = sampled_past_timestamps_to_tensor(past_time_stamps) #时间戳转换成张量
    ...
    def get_ego_agent_future(self):
        ...
        # Get all future poses of the ego relative to the ego coordinate system，坐标系最终都要为自车坐标系
    ...
    # 获取自车候选轨迹，用于轨迹树扩展---只是用于轨迹树算法，其他可以不用
    ...
    def plot_scenario(self, data):
        ...
        plt.gca().set_aspect('equal')# 设置横纵轴等比例，避免地图/轨迹变形
